-- ELEV8 Database Schema - INFINITE RECURSION FIX (FINAL SECURE VERSION)
-- This script fixes the infinite recursion issue in RLS policies
-- AND addresses security warnings by setting search_path
-- FIXES: Properly drops existing functions before recreating them
...
-- Step 2: Drop ALL existing functions first (including the problematic ones)
...
-- Step 4: Recreate other functions with secure search_path and correct signatures
...
-- Recreate get_user_organizations function with proper signature
...
-- Recreate create_organization_with_admin function
...
SELECT 'All functions created successfully with secure search_path and proper signatures. Test with: SELECT public.is_app_admin();' as status;
